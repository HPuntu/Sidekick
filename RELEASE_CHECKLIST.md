...
- [ ] Enable web fetch, add an explicit HTTPS host allowlist, and use `@url(...)` with an allowed public host.
- [ ] Confirm HTTP, empty allowlist, localhost/private URL, and private DNS resolution fetches are blocked.

## Safety QA

- [ ] Keep Pi tools disabled and confirm prompts include `--no-tools`.
- [ ] Keep experimental Pi features disabled and confirm prompts include Pi disabling flags for extensions, skills, prompt templates, and context files.
- [ ] Enable experimental Pi features only in a test vault and confirm the disabling flags are removed.
...
- [ ] Confirm GitHub Actions workflow permissions are set to read/write for release publishing.
- [ ] Confirm any `main` branch protection permits the release workflow to push its version commit and tag.
- [ ] Merge `dev` into `main` and confirm the main-branch release workflow publishes the versioned release.
